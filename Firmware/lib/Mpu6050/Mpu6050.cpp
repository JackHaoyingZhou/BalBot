...
{
	// I2C Interface
	this->i2c_device = I2CDevice(wire, i2c_addr, I2CDevice::msb_first);
...
void Mpu6050::init()
{
	i2c_device.write_uint8(reg_pwrmgmt_addr, reg_pwrmgmt_wake);
}
 ...
 * Must call 'calibrate' function first to compute this.
 */
...
void Mpu6050::update()
{
	...
	i2c_device.read_seq(reg_data_addr, 14);

	// Read accelerometer
	acc_x = i2c_device.read_int16() * acc_per_cnt;
	acc_y = i2c_device.read_int16() * acc_per_cnt;
	acc_z = i2c_device.read_int16() * acc_per_cnt;

	// Toss out temperature
	i2c_device.read_uint16();

	// Read and adjust gyroscope
	vel_x = i2c_device.read_int16() * vel_per_cnt - vel_x_cal;
	vel_y = i2c_device.read_int16() * vel_per_cnt - vel_y_cal;
	vel_z = i2c_device.read_int16() * vel_per_cnt - vel_z_cal;
}
...
/**
 * @brief Returns xz acceleration [m/s^2] from most recent update.
 */
